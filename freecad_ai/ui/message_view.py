...
def get_freecad_mode_name() -> str:
    """Return FreeCAD's current UI mode/theme name.
    ...
    try:
        ...
        if theme:
            if theme != _LAST_REPORTED_THEME:
                _LAST_REPORTED_THEME = theme
            return theme

        fallback = "Custom/Unknown"
        if fallback != _LAST_REPORTED_THEME:
            App.Console.PrintMessage("FreeCAD theme not set, using fallback: Custom/Unknown\\n")
            _LAST_REPORTED_THEME = fallback
        return fallback
    except Exception as e:
        try:
            import FreeCAD as App
            App.Console.PrintWarning(
                f"Failed to get FreeCAD theme: {e}, using fallback: Custom/Unknown\\n"
            )
        except Exception:
            pass
...
def _get_theme_colors() -> dict:
    """Return hardcoded colors selected by FreeCAD mode."""
    if _is_dark_mode(get_freecad_mode_name()):
